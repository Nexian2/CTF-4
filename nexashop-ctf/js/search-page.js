...
// MTqjoUR6IzgfnT9aDGx=
window._nxs = { _authref: "MTqjoUR6IzgfnT9aDGx=" };

function renderProducts(products) {
  const grid = document.getElementById('productsGrid');
  if (!products.length) {
    grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:60px;color:var(--grey-400);font-size:.875rem;">No results found.</div>';
    return;
  }
...
function init() {
  ...
  if (q) {
    ...
    // VULNERABLE: directly inserted into innerHTML without sanitization
    heading.innerHTML = '<h1>Search results for: <span class="search-query-label">' + q + '</span></h1>';
    ...
    meta.textContent = 'Showing results for "' + q + '"';
    renderProducts(filtered);
  } else if (cat) {
    heading.innerHTML = '<h1>' + cat + '</h1>';
    const filtered = PRODUCTS.filter(p => p.category === cat);
    document.getElementById('resultsCount').textContent = filtered.length + ' products found';
    renderProducts(filtered);
  } else {
    heading.innerHTML = '<h1>All Products</h1>';
